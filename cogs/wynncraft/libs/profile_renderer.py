...
def generate_profile_card(info, output_path="profile_card.png", skin_image=None):
    ...
    rank_right_edge_x = 450
    normal_left_x = 440
    graid_right_edge_x = 630
    ...
    for label, val, x, y in raid_stats_list:
        ...
        val_w = bbox[2] - bbox[0]
        draw.text((x + 315 - val_w, y), val_str, font=font_mini, fill=(60,40,30,255))
    ...
    if top_ranks:
        for i, rank_data in enumerate(top_ranks):
            y_pos = rank_base_y + 45 + (i * 55)
            draw.text((rank_base_x, y_pos), f"{rank_data['category']}:", font=font_mini, fill=(60, 40, 30, 255))
            
            rank_str = f"#{fmt_num(rank_data['rank'])}"
            bbox = draw.textbbox((0,0), rank_str, font=font_mini)
            draw.text((370 - (bbox[2] - bbox[0]), y_pos + 26), rank_str, font=font_mini, fill=(60, 40, 30, 255))
    else:
        draw.text((rank_base_x, rank_base_y + 45), "API Hidden", font=font_mini, fill=(60, 40, 30, 255))
    ...
    if uuid and '-' in uuid:
        ...
        line1 = line2 = ""
    draw.text((700, 1300), "UUID", font=font_mini, fill=(90,90,90,255))
    draw.text((700, 1340), line1 + "-", font=font_mini, fill=(90,90,90,255))
    draw.text((700, 1380), line2, font=font_mini, fill=(90,90,90,255))

    draw.text((700, 1450), "Generated by", font=font_mini, fill=(60,40,30,255))
    draw.text((735, 1490), "AtamaWaruiBoi", font=font_mini, fill=(60,40,30,255))
    draw.text((770, 1530), "#3244", font=font_mini, fill=(60,40,30,255))
