...
def generate_profile_card(info, output_path="profile_card.png", skin_image=None):
    ...
    if top_ranks:
        for i, rank_data in enumerate(top_ranks):
            ...
            draw.text((rank_base_x-20, y_pos), f"{rank_data['category']}:", font=font_mini, fill=(60, 40, 30, 255))
            
            rank_str = f"#{fmt_num(rank_data['rank'])}"
            bbox = draw.textbbox((0,0), rank_str, font=font_mini)
            draw.text((365 - (bbox[2] - bbox[0]), y_pos + 30), rank_str, font=font_mini, fill=(60, 40, 30, 255))
    else:
    ...
    if uuid and '-' in uuid:
        ...
        line1 = line2 = ""
    draw.text((700, 1150), "UUID", font=font_mini, fill=(90,90,90,255))
    draw.text((700, 1180), line1 + "-", font=font_tiny, fill=(90,90,90,255))
    draw.text((700, 1210), line2, font=font_tiny, fill=(90,90,90,255))

    draw.text((700, 1300), "Generated by", font=font_mini, fill=(60,40,30,255))
    draw.text((735, 1330), "AtamaWaruiBoi", font=font_mini, fill=(60,40,30,255))
    draw.text((770, 1360), "#3244", font=font_mini, fill=(60,40,30,255))
