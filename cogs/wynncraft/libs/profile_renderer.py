...
def generate_profile_card(info, output_path="profile_card.png", skin_image=None):
    ...
    for label, t_key, g_key, r_key, y in raid_keys:
        ...
        if r_key:
            ...
            r_w = bbox[2] - bbox[0]
            draw.text((rank_right_edge_x - r_w, y+2), rank_text, font=font_vtiny, fill=(60,40,30,255))
    ...
    if top_ranks:
        for i, rank_data in enumerate(top_ranks):
            y_pos = rank_base_y + 55 + (i * 55)
            draw.text((rank_base_x, y_pos), f"{rank_data['category']}:", font=font_tiny, fill=(60, 40, 30, 255))
            
            rank_str = f"#{fmt_num(rank_data['rank'])}"
            bbox = draw.textbbox((0,0), rank_str, font=font_tiny)
            draw.text((365 - (bbox[2] - bbox[0]), y_pos + 30), rank_str, font=font_tiny, fill=(60, 40, 30, 255))
    else:
    ...
    if uuid and '-' in uuid:
        ...
        line1 = line2 = ""
    draw.text((690, 1150), "UUID", font=font_mini, fill=(90,90,90,255))
    draw.text((690, 1180), line1 + "-", font=font_tiny, fill=(90,90,90,255))
    draw.text((690, 1210), line2, font=font_tiny, fill=(90,90,90,255))

    draw.text((690, 1300), "Generated by", font=font_mini, fill=(60,40,30,255))
    draw.text((725, 1330), "AtamaWaruiBoi", font=font_mini, fill=(60,40,30,255))
    draw.text((830, 1360), "#3244", font=font_mini, fill=(60,40,30,255))
